...
func main() {
	...
	fmt.Println("3. Connect with EuroScope (multiple clients supported!)")
	...
	fmt.Println("- Connect multiple EuroScope clients (e.g., TWR + GND)")
	fmt.Println("- Each client's events are tagged with their callsign")
	fmt.Println("- Client connect/disconnect events are automatically recorded")
	fmt.Println()
