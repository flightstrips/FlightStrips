...
func Build(ctx context.Context, cfg Config, deps Dependencies) (*App, error) {
	...
	if amanRuntime.Enabled() {
		...
		amanCoordination = coordinationrequest.NewService(repository, repository)
	}
	if amanRuntime.Enabled() {
	...
	realtime, err := assembleRealtime(stripService, controllerService, authService, amanStateProvider, amanCommands, amanCoordination, amanRouteFacts, amanRuntime.Ownership().ControllerMutationAuthorized, amanRuntime.Ownership().EuroScopeGainLoseTagsEnabled, amanRuntime.Ownership().HoldingEATWritebackEnabled)
	if err != nil {
		if closeDB {
			dbpool.Close()
		}
...
func assembleRealtime(stripService shared.StripService, controllerService shared.ControllerService, authService shared.AuthenticationService, amanState frontend.AMANStateProvider, amanCommands aman.CommandService, amanCoordination *coordinationrequest.Service, amanRouteFacts euroscope.AMANRouteFactReporter, amanMutations bool, amanGainLossEnabled bool, holdingEATEnabled bool) (realtimeAssembly, error) {
	frontendHub, err := frontend.NewHub(frontend.HubDependencies{
		Strips: stripService, Authentication: authService, AMANState: amanState, AMANCommands: amanCommands, AMANCoordination: amanCoordination, AMANMutations: amanMutations,
	})
	if err != nil {
	...
	euroscopeHub, err := euroscope.NewHub(euroscope.HubDependencies{
		Strips: stripService, Controllers: controllerService, Authentication: authService, AMANGainLoss: asAMANGainLossProvider(amanState, amanGainLossEnabled), AMANHoldingEAT: asAMANHoldingEATProvider(amanState, holdingEATEnabled), AMANRouteFacts: amanRouteFacts,
	})
	if err != nil {
...
func asAMANHoldingEATProvider(provider frontend.AMANStateProvider, enabled bool) euroscope.AMANHoldingEATProvider {
	if provider == nil || !enabled {
		return nil
	}
	result, _ := provider.(euroscope.AMANHoldingEATProvider)
	return result
}

func asAMANGainLossProvider(provider frontend.AMANStateProvider, enabled bool) euroscope.AMANGainLossProvider {
	if provider == nil || !enabled {
		return nil
	}
	result, _ := provider.(euroscope.AMANGainLossProvider)
	return result
}
