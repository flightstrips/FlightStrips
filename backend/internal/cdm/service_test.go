...
func TestHandleReadyRequest_UsesReadyFlow(t *testing.T) {
	...
	assert.Zero(t, readCount, "READY synchronization must not wait for a vIFF readback")
}

func TestHandleReadyRequest_RetriesReaBeforeExportAndClearsPending(t *testing.T) {
	const sessionID = int32(216)
	const callsign = "SAS216"

	var requests []string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/ifps/dpi" {
			t.Fatalf("unexpected path %s", r.URL.Path)
		}
	...
	err := service.HandleReadyRequest(context.Background(), sessionID, callsign, "EKCH_DEL", "ATC")
	require.Error(t, err)
	assert.True(t, stored.ReadySyncPending)
	require.Equal(t, []string{"REA/1"}, requests)
...
func TestHandleReadyRequest_DoesNotRequireAtcEobtUpdate(t *testing.T) {
...
func TestHandleTobtUpdate_PersistsAuthoritativeOverride(t *testing.T) {
...
func TestHandleTobtUpdate_EarlierValueRequestsImprovement(t *testing.T) {
	...
	assert.False(t, persisted.TobtAutoSynced)
	assert.True(t, persisted.Recalculate)
	assert.Equal(t, models.CdmRecalculationImproveOnly, persisted.RecalculationMode)
...
func TestHandleClxTobtUpdate_BroadcastsOtherFlightsMovedByRecalculation(t *testing.T) {
	const sessionID = int32(45)
	const airport = "EKCH"
	const runway = "22L"
	const editedCallsign = "SASEDIT"
	const pendingCallsign = "SASPENDING"

	now := time.Now().UTC().Truncate(time.Minute)
	editedTobt := now.Add(20 * time.Minute).Format("1504")
	pendingTobt := now.Add(21 * time.Minute).Format("1504")
	euroscopeSeenAt := now
	stored := map[string]*models.CdmData{
		editedCallsign:  (&models.CdmData{Eobt: stringPtr(editedTobt)}).Normalize(),
		pendingCallsign: (&models.CdmData{Tobt: stringPtr(pendingTobt), Recalculate: true, RecalculationMode: models.CdmRecalculationRequired}).Normalize(),
	}
	stripForState := func(callsign string) *models.Strip {
		return &models.Strip{
			Callsign:        callsign,
			Session:         sessionID,
			Origin:          airport,
			Runway:          stringPtr(runway),
			EuroscopeSeenAt: &euroscopeSeenAt,
			CdmData:         stored[callsign].Clone(),
		}
	}

	stripRepo := &testutil.MockStripRepository{
		GetByCallsignFn: func(_ context.Context, _ int32, callsign string) (*models.Strip, error) {
			return stripForState(callsign), nil
		},
		GetCdmDataForCallsignFn: func(_ context.Context, _ int32, callsign string) (*models.CdmData, error) {
			return stored[callsign].Clone(), nil
		},
		ListByOriginFn: func(context.Context, int32, string) ([]*models.Strip, error) {
			return []*models.Strip{stripForState(editedCallsign), stripForState(pendingCallsign)}, nil
		},
	...
	sessionRepo := &testutil.MockSessionRepository{GetByIDFn: func(context.Context, int32) (*models.Session, error) {
		return &models.Session{ID: sessionID, Airport: airport, ActiveRunways: pkgModels.ActiveRunways{DepartureRunways: []string{runway}}}, nil
	}}
	frontendHub := &testutil.MockFrontendHub{}
	euroscopeHub := &testutil.MockEuroscopeHub{}
	...
	if err := service.HandleClxTobtUpdate(context.Background(), sessionID, editedCallsign, editedTobt, "EKCH_B_GND", "ATC"); err != nil {
		t.Fatalf("HandleClxTobtUpdate returned error: %v", err)
	}
	if len(frontendHub.CdmUpdates) != 2 {
		t.Fatalf("expected both recalculated flights to be published, got %#v", frontendHub.CdmUpdates)
	}
	if len(euroscopeHub.Broadcasts) != 2 {
		t.Fatalf("expected both recalculated flights to reach EuroScope, got %#v", euroscopeHub.Broadcasts)
	}
}
...
func TestHandleReadyRequest_PreservesTobtWithinTsatWindow(t *testing.T) {
	...
	require.NotNil(t, persisted.Tobt)
	assert.Contains(t, []string{beforeNow, afterNow}, *persisted.Tobt)
	assert.Equal(t, activeTsat, valueOrEmpty(persisted.Tsat))
	require.NotNil(t, persisted.Status)
	assert.Equal(t, "REA", *persisted.Status)
}
...
func TestHandleReadyRequest_UpdatesPastTobtToNowWithActiveTsat(t *testing.T) {
	...
	assert.Equal(t, previousAsrt, *persisted.Asrt)
	require.NotNil(t, persisted.Status)
	assert.Equal(t, "REA", *persisted.Status)
}

func TestHandleReadyRequest_AdoptsEarlierCandidate(t *testing.T) {
	...
	require.NotNil(t, stored.Tsat)
	assert.True(t, isEarlierClock(*stored.Tsat, previousTsat))
	require.NotNil(t, stored.Ttot)
	assert.True(t, isEarlierClock(*stored.Ttot, previousTtot))
	...
	assert.NotEmpty(t, lastUpdate.Tsat)
...
func TestPushViffDataAfterRecalc_DoesNotBypassPendingReadyOrdering(t *testing.T) {
	...
	tobt, tsat, ttot := "1525", "153000", "154000"
...
func TestTriggerRecalculate_RunsForEverySession(t *testing.T) {
...
func TestTriggerRecalculateForAirport_RunsAllMatchingSessions(t *testing.T) {
	...
	case sessionID := <-listByOriginCalled:
		assert.Contains(t, []int32{55, 57}, sessionID)
	case <-time.After(time.Second):
	...
	case sessionID := <-listByOriginCalled:
		assert.Contains(t, []int32{55, 57}, sessionID)
	case <-time.After(time.Second):
		t.Fatal("expected every matching session to be recalculated")
	}
}
...
func TestSyncSessions_TriggersImmediateRecalculateForPersistedMasterSession(t *testing.T) {
	...
	sessionRepo := &testutil.MockSessionRepository{
		ListFn: func(_ context.Context) ([]*models.Session, error) {
			return []*models.Session{
				{ID: sessionID, Name: "SWEATBOX", Airport: airport},
			}, nil
		},
		GetByIDFn: func(_ context.Context, id int32) (*models.Session, error) {
...
func TestSyncSessions_DoesNotUseViffForNonLiveMasterSession(t *testing.T) {
	...
	sessionRepo := &testutil.MockSessionRepository{
		ListFn: func(_ context.Context) ([]*models.Session, error) {
			return []*models.Session{
				{ID: sessionID, Name: "SWEATBOX", Airport: "EKCH"},
			}, nil
		},
	}
