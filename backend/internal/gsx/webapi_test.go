...
func newAPI(sessions *sessionFake, strips *stripFake) *WebAPI {
	return NewWebAPI(sessions, strips, true)
}
...
func TestHandleStandDisclosesOnlyStandAndRevision(t *testing.T) {
	strips := &stripFake{strips: map[int32]map[string]*models.Strip{
		1: {"SAS1401": {
			Callsign:    "SAS1401",
			Stand:       ptr("A12"),
	...
	if len(payload) != 2 {
		t.Fatalf("expected exactly stand and revision, got %v", payload)
	}
	for _, key := range []string{"stand", "revision"} {
