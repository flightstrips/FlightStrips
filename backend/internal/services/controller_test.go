...
func TestControllerOffline_MismatchedPrefixCoverageDoesNotSuppressOfflineHandling(t *testing.T) {
	ctx := context.Background()
	t.Cleanup(config.SetPositionsForTest([]config.Position{
		{Name: "EKCH_TWR", Frequency: "118.700", Section: "TWR"},
	}))
	...
	ctrlRepo := &testutil.MockControllerRepository{
		...
		GetByPositionFn: func(_ context.Context, _ int32, pos string) ([]*models.Controller, error) {
			...
			return []*models.Controller{
				existingController,
				{Callsign: "ESMS_TWR", Session: session, Position: position},
			}, nil
		},
	}
...
func TestControllerOnline_New_IgnoresMismatchedPrefixCoverageForSingleOnPosition(t *testing.T) {
	ctx := context.Background()
	t.Cleanup(config.SetPositionsForTest([]config.Position{
		{Name: "EKCH_TWR", Frequency: "118.700", Section: "TWR"},
	}))
