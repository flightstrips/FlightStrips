...
### Position-only batch diagnostics (2026-09-16)
...
The position-only implementation is committed as `1d205c18`. Sequential diagnostic runs
...
The full PostgreSQL suite and affected race tests passed, including AMAN
operational observations. Focused coverage verifies 100 established identities
with one query and one execution slot; CID changes, new callsigns, retirement and
fresh subsequent reports; simultaneous creation of one active callsign; and
invalid/cancelled members without losing healthy results. Load reports expose
`batch_aman_identity_*` sizes/timings, grouped identity SQL, and remaining
individual identity SQL separately.

The follow-up used the same local conditions, one worker, ten-second warm-up,
two-minute measurement and overload/recovery as the position-only runs above:

| Wire order | P95 before / after | P99 before / after | DB operations/report before / after |
|---|---:|---:|---:|
| [Controls interleaved](performance/2026-09-16/aman-batch-interleaved-1-worker.json) | 63.45 / 37.45 ms | 69.68 / 63.36 ms | 0.703 / 0.281 |
| [Controls after positions](performance/2026-09-16/aman-batch-after-burst-1-worker.json) | 47.53 / 30.61 ms | 63.42 / 60.52 ms | 0.628 / 0.174 |

For the interleaved case, AMAN identity SQL statements fell from 5,404 to 333
(300 bulk reads plus 33 individual reads). Their combined SQL time fell from
2,011.47 ms to 186.60 ms, a 90.7% reduction. Bulk identity reads averaged 17.90
identities. In the after-burst case, identity SQL time fell from 1,987.97 ms to
138.92 ms (93.0%), with 128 bulk reads averaging 40.80 identities plus 181
individual reads. These are SQL work totals, not per-report latency percentiles.

Both runs completed exactly 15,200 reports with zero position/operational errors,
...
handlers: heading processing averaged 3.38 ms and the four ground-state handlers
averaged 26.05 ms in the interleaved run. No extra position workers, cross-message
