...
import { createWebSocketStore, type WebSocketState } from "./store.js";
import { WebSocketClient } from "../api/websocket.js";

// Create a context for the WebSocket store
const WebSocketStoreContext = createContext<ReturnType<
  typeof createWebSocketStore
> | null>(null);
...
export const useWebSocketStore = <T,>(
  ...
  if (!store) {
    throw new Error(
      "useWebSocketStore must be used within a WebSocketStoreProvider"
    );
  }
...
export const useControllers = () =>
  useWebSocketStore((state) => state.controllers);
...
export const useRunwaySetup = () =>
  useWebSocketStore((state) => state.runwaySetup);

export const useStrip = (callsign: string) =>
  useWebSocketStore((state) =>
    state.strips.find((strip) => strip.callsign === callsign)
  );
