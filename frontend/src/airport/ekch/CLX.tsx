import { FlightStrip } from "@/components/refactor/strip/FlightStrip";
import { Message } from "@/components/refactor/Message";
import {
  useClearedStrips,
...
export default function DEL() {
  ...
  const norgewianStrips = useNorwegianBayStrips().sort(
    (a, b) => a.sequence - b.sequence
  );
  const otherStrips = useOtherBayStrips().sort(
    (a, b) => a.sequence - b.sequence
  );
  ...
  return (
    <>
      <div className="bg-[#A9A9A9] w-screen h-[calc(100vh-4rem)] flex justify-center justify-items-center gap-2 aspect-video">
        ...
        <div className="w-1/4 h-full bg-[#555355]">
          <div className="bg-[#393939] h-10 flex items-center px-2 justify-between">
            <span className="text-gray-100 font-bold text-lg">CLEARED</span>
          </div>
          ...
          <div className="h-[calc(50%-6rem)] w-full bg-[#555355]">
            <Message>
              ...
              tincidunt vitae enim eget porttitor. Suspendisse ultrices
              ullamcorper tortor, vitae condimentum lacus convallis at.{" "}
            </Message>
            <Message>
              <b>FLIGHTSTRIPS</b> has deteced that EKCH_DEL has logged off. You
              are in change of Delivery!
            </Message>
