...
export default function DEL() {
  const sasStrips = useSasBayStrips();
  const norgewianStrips = useNorwegianBayStrips();
  const otherStrips = useOtherBayStrips();
  const cleared = useClearedStrips();

  const mapToStrip = (strip: FrontendStrip, status: string) => <FlightStrip callsing={strip.callsign} destination={strip.destination} stand={strip.stand} eobt={strip.eobt} status={status} key={strip.callsign}/>
