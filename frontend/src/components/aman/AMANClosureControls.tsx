...
export function AMANClosureControls({disabled, groups, flights, pending, onCommand}: {disabled: boolean; groups: AMANRunwayGroup[]; flights: AMANFlight[]; pending: AMANPendingCommand[]; onCommand: (intent: AMANCommandIntent) => void}) {
  ...
  const valid = Boolean(reason.trim() && (mode === "absolute" ? utc(start) : anchors.some((flight) => flight.callsign === after)) && (!end || utc(end)));
  const send = () => {
    ...
    else if (after) onCommand({type: "aman.create_runway_closure", runway_group_id: selected, after_callsign: after, end: endAt, reason: reason.trim()});
  };
