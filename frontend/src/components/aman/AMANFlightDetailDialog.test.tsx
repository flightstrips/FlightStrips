...
function Harness() {
  ...
  return <><button onClick={() => setOpen(true)} type="button">Open SAS123</button>{open && <AMANFlightDetailDialog airport="EKCH" callsign="SAS123" onClose={() => setOpen(false)} />}</>;
}
...
describe("AMAN flight detail dialog integration", () => {
  ...
  it("requires confirmation before reporting a missed approach", async () => {
    ...
    render(<AMANFlightDetailDialog airport="EKCH" callsign="SAS123" missedApproach={{
  ...
  it("shows detector evidence and pending, rejected, and server-confirmed states without relying on color", async () => {
    ...
    const {rerender} = render(<AMANFlightDetailDialog airport="EKCH" callsign="SAS123" missedApproach={props} onClose={vi.fn()} />);
    await screen.findByRole("dialog", {name: /SAS123/});
    await screen.findByText("A320");
    fireEvent.click(screen.getByRole("button", {name: "Missed approach"}));
    ...
    rerender(<AMANFlightDetailDialog airport="EKCH" callsign="SAS123" missedApproach={{...props, pending: false, rejection: null, confirmed: true}} onClose={vi.fn()} />);
  ...
  it("requires a focused, explicitly labeled confirmation before removal", async () => {
    fetchDetail.mockResolvedValue(detail);
    const onConfirm = vi.fn();
    render(<AMANFlightDetailDialog airport="EKCH" callsign="SAS123" removal={{
  ...
  it("shows authorization, pending, rejection, and server confirmation without relying on color", async () => {
    ...
    const {rerender} = render(<AMANFlightDetailDialog airport="EKCH" callsign="SAS123" removal={props} onClose={vi.fn()} />);
    await screen.findByRole("dialog", {name: /SAS123/});
    await screen.findByText("A320");
    fireEvent.click(screen.getByRole("button", {name: "Remove from AMAN"}));
    ...
    rerender(<AMANFlightDetailDialog airport="EKCH" callsign="SAS123" removal={{...props, blockReason: null, pending: true, rejection: {code: "stale_revision", message: "state changed"}}} onClose={vi.fn()} />);
    expect(screen.getByText("Waiting for server confirmation")).toBeInTheDocument();
    expect(screen.getByRole("alert")).toHaveTextContent("Rejected: state changed (stale_revision)");

    rerender(<AMANFlightDetailDialog airport="EKCH" callsign="SAS123" removal={{...props, blockReason: null, confirmed: true}} onClose={vi.fn()} />);
