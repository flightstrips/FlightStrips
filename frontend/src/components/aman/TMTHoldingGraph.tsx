...
function entryState(position: ReturnType<typeof layoutHoldingGraph>[number]) {
  ...
  return {label: "SCHEDULED", symbol: "", tone: "border-[#f0e129] text-[#f0e129]"};
}
...
export function TMTHoldingGraph({entries, now = new Date()}: {entries: AMANHoldingEntry[]; now?: Date}) {
  ...
  return (
    <section aria-label="TMT holding information" className="flex flex-col border border-[#777] bg-[#292929] text-[#dcdcdc]">
      ...
      <div className="relative mx-2 h-[500px] min-h-[420px] bg-[#3c3c3c]" data-testid="holding-graph">
        ...
        <ul className="absolute inset-3">
          {placed.map((position) => {
            ...
            return (
              <li
                aria-label={`${position.entry.callsign}, ${position.entry.holding}, ${position.altitude.label}, EAT ${position.time.label}, ${state.label}`}
                className="absolute left-[34%] grid w-[51%] -translate-y-1/2 grid-cols-[54px_1fr_52px] border border-[#cdcdcd] bg-[#3c3c3c] font-mono text-[10px] font-semibold"
              ...
              >
                <span className={cn("flex items-center justify-center border-r border-[#cdcdcd] px-1", state.tone)} title={state.label}>
                  {state.symbol && <b className="mr-1 text-[8px]">{state.symbol}</b>}{compactTime(position.time.label)}
                </span>
                <span className="truncate px-1.5 py-1 text-center">{position.entry.callsign}<small className="ml-1 text-[8px] text-slate-300">{position.entry.holding}</small></span>
      ...
      {unplaced.length > 0 && (
        <div className="mt-8 border-t border-[#777] px-2 py-2 text-[10px]" aria-label="Holding aircraft with missing values">
          {unplaced.map(({entry, time, altitude}) => <div className="grid grid-cols-[1fr_55px_55px_55px] gap-1 font-mono" key={entry.flight_id}><b>{entry.callsign}</b><span>{entry.holding}</span><span>EAT {time.label}</span><span>CFL {altitude.label}</span></div>)}
        </div>
      )}
      <footer className="flex gap-3 border-t border-[#777] px-2 py-1.5 text-[9px] font-semibold">
        <span className="text-[#62e562]">≤4 DUE ≤4 MIN</span><span className="text-[#f0e129]">SCHEDULED</span><span className="text-[#ff8b8b]">! PAST EAT</span>
      </footer>
    </section>
  );
}
