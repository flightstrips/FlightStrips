...
func Test_euroscopehandlerAuthentication(t *testing.T) {
	...
	_, err := server.euroscopehandlerAuthentication(emptyTokenEvent, authServerURL)
	assert.Error(t, err)
	...
	token, err := server.euroscopehandlerAuthentication(correctTokenEvent, authServerURL)
