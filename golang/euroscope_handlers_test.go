...
func Test_euroscopehandlerAuthentication(t *testing.T) {
	...
	_, err := server.euroscopeeventhandlerAuthenticationTokenValidation("", authServerURL)
	assert.Error(t, err)
	...
	token, err := server.euroscopeeventhandlerAuthenticationTokenValidation(expiredToken, authServerURL)
